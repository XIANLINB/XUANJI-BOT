 ...
 * 保证控制台新增的 XuanJiBot 重启后能自动拉起。yaml 已声明的 appId 优先，不会被库覆盖。
 *
...
public class XuanJiBotRunner implements CommandLineRunner {
    ...
    private final ObjectProvider<XuanJi.core.config.ConfigService> configService;
     ...
     *
     * <p>只合并 status=ONLINE 且 secret 非空的记录，避免把停用的 XuanJiBot 误拉起来。
     */
    ...
    private boolean resolveNewOpenBot() {
        try {
            XuanJi.core.config.ConfigService cs = configService.getIfAvailable();
            if (cs != null) {
    ...
    private void registerAndStart(String botKey, XuanJiRobotProperties.RobotProperties config, boolean isNewOpenBot) {
        ...
        try {
            ...
            log.warn("[启动] 写入 XuanJiBot 注册信息失败: {}", e.getMessage());
        }
    }
}
