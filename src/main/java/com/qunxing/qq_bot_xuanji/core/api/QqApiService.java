...
import com.qunxing.qq_bot_xuanji.common.config.QqPlatformConfig;

import com.qunxing.qq_bot_xuanji.registry.AccessTokenService;
import com.qunxing.qq_bot_xuanji.registry.RobotRegistry;

import com.qunxing.qq_bot_xuanji.common.exception.BusinessException;
 ...
 *   <li><b>错误解析</b> — 使用 {@link com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode}
 ...
 *       如 {@link #post(Long, String, String, JSONObject)}</li>
 *   <li><b>通过凭证调用</b> — 手动传入 appId 和 appSecret，
 *       如 {@link #post(String, String, String, String, JSONObject)}</li>
 * </ul>
 *
 * <h3>使用示例</h3>
 * <pre>{@code
 ...
 * body.put("msg_type", 0);
 * JSONObject resp = qqApiService.post(robotId, "SANDBOX",
 *     "/v2/groups/{group_openid}/messages", body);
 *
 * // 查询频道信息（通过凭证）
 * JSONObject info = qqApiService.get(appId, appSecret, "PRODUCTION",
 *     "/v2/guilds/{guild_id}");
 * }</pre>
 *
 * @see AccessTokenService  AccessToken 生命周期管理
 * @see RobotRegistry       机器人凭证查找
 * @see com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode QQ 平台错误码枚举
 */
@Slf4j
@Service
public class QqApiService {
     ...
     * @return 响应 JSON
     */
    public JSONObject post(Long robotId, String envType, String path, JSONObject body) {
     ...
     * @return 响应 JSON
     */
    public JSONObject postWithTimeout(Long robotId, String envType, String path, JSONObject body, int timeoutSeconds) {
        String[] creds = getCredentialOrThrow(robotId, envType);
     ...
     * @return 响应 JSON
     */
    public JSONObject put(Long robotId, String envType, String path, JSONObject body) {
        String[] creds = getCredentialOrThrow(robotId, envType);
     ...
     * @return 响应 JSON
     */
    public JSONObject get(Long robotId, String envType, String path) {
        String[] creds = getCredentialOrThrow(robotId, envType);
        // 使用全局配置的 isNewOpenBot
     ...
     * @return 响应 JSON
     */
    public JSONObject delete(Long robotId, String envType, String path) {
        String[] creds = getCredentialOrThrow(robotId, envType);
        // 使用全局配置的 isNewOpenBot
     ...
     * @return 响应 JSON
     */
    public JSONObject post(String appId, String appSecret, String envType,
     ...
     */
    public JSONObject put(String appId, String appSecret, String envType,
                          String path, JSONObject body) {
     ...
     */
    public JSONObject patch(String appId, String appSecret, String envType,
                            String path, JSONObject body) {
     ...
     */
    public JSONObject delete(String appId, String appSecret, String envType,
                             String path) {
     ...
     * GET 请求（通过凭证）
     */
    public JSONObject get(String appId, String appSecret, String envType,
                          String path) {
     ...
     */
    private JSONObject sendRequest(String method, String appId, String appSecret,
                                   String envType, String path, JSONObject body,
     ...
     * <p>用于媒体上传等需要较长超时的请求。
     */
    private JSONObject sendRequestWithTimeout(String method, String appId, String appSecret,
                                               String envType, String path, JSONObject body,
        ...
        try {
            ...
            if (statusCode >= 200 && statusCode < 300) {
                if (responseBody == null || responseBody.isEmpty()) return new JSONObject();
                return new JSONObject(responseBody);
            } else {
     ...
     */
    private JSONObject sendRequestOnce(String method, String appId, String appSecret,
                                       String envType, String path, JSONObject body,
     ...
     * 并使用 {@link com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode} 枚举
     * 提供结构化的错误信息和排查建议。
     *
    ...
    private String parseErrorMessage(String responseBody, int statusCode) {
        ...
        try {
            ...
            int code = errorJson.optInt("code", statusCode);
            String message = errorJson.optString("message", "未知错误");
            String traceId = errorJson.optString("trace_id", "");

            // 查找错误码枚举，获取排查建议
            var errorCode = com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode.of(code);
            if (errorCode != null) {
     ...
     */
    public static com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode parseErrorCode(String responseBody) {
        if (responseBody == null || responseBody.isEmpty()) return null;
        try {
            JSONObject errorJson = new JSONObject(responseBody);
            int code = errorJson.optInt("code", 0);
            return com.qunxing.qq_bot_xuanji.common.enums.QqApiErrorCode.of(code);
        } catch (Exception e) {
            return null;
        }
    }
