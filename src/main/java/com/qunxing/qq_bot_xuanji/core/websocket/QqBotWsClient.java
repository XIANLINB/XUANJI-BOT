package com.qunxing.qq_bot_xuanji.core.websocket;

import com.qunxing.qq_bot_xuanji.event.EventDispatcher;
 ...
 *   <li><b>事件接收</b> — 收到 op=0 的 Dispatch 事件，分发给 {@link EventDispatcher}</li>
 * </ol>
 *
 * <h3>重连机制</h3>
 ...
 * @see EventDispatcher 处理从 WebSocket 接收到的事件
 */
@Slf4j
public class QqBotWsClient {
    ...
    public QqBotWsClient(Long robotId, String envType, String appId, String appSecret,
                          String gatewayUrl, EventDispatcher eventDispatcher,
    ...
    private void sendIdentify() {
        ...
        d.put("shard", new org.json.JSONArray().put(0).put(1));  // 分片：[当前分片, 总分片数]
        d.put("properties", new JSONObject());    // 连接属性（可选）
        identify.put("d", d);
    ...
    private void handleHello(WsPayload payload) {
        int heartbeatInterval = payload.getD().optInt("heartbeat_interval", 41250);
     ...
     * 否则将事件交给 {@link EventDispatcher} 分发到对应的 Handler。
     *
    ...
    private void handleDispatch(WsPayload payload) {
        ...
        // 提取事件 ID 并分发给 EventDispatcher
        String eventId = data.optString("id", "");
        totalEvents++;
