...
import XuanJi.adapter.qqbot.api.QqApiService;
...
/**
 * 单个机器人的 QQ XuanJiBot WebSocket 客户端
 *
...
public class QqBotWsClient {
    ...
    private void handleDispatch(WsPayload payload) {
        ...
        // rawEventType / envType 作为 XuanJiEvent 一等字段携带，不再污染原始 data
        try {
            XuanJiBot bot = new XuanJiBot("qq:" + appId, "qq", appId, XuanJi.api.adapter.XuanJiBot.Status.ONLINE, java.util.Set.of());
            XuanJi.api.event.XuanJiEvent be = XuanJi.adapter.qqbot.converter.QqEventConverter.convert(bot, eventType, envType, data, eventId);
            log.info("[XuanJiEvent] 已转换: type={}, user={}, group={}, text={}",
