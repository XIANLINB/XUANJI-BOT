 ...
 *   <li><b>错误解析</b> — 使用 {@link XuanJi.adapter.qqbot.enums.QqApiErrorCode}
 ...
 * @see XuanJi.adapter.qqbot.enums.QqApiErrorCode QQ 平台错误码枚举
 */
@Slf4j
@Service
public class QqApiService {
    ...
    private String[] getCredentialOrThrow(String robotId, String envType) {
        ...
        log.info("[QQ鉴权] robotId={} → id={}, botId={}, appId={}, env={}",
                robotId, robot.getId(), robot.getBotId(), robot.getAppId(), envType);
     ...
     * 并使用 {@link XuanJi.adapter.qqbot.enums.QqApiErrorCode} 枚举
     * 提供结构化的错误信息和排查建议。
     *
    ...
    private String parseErrorMessage(String responseBody, int statusCode) {
        ...
        try {
            ...
            // 查找错误码枚举，获取排查建议
            var errorCode = XuanJi.adapter.qqbot.enums.QqApiErrorCode.of(code);
            if (errorCode != null) {
     ...
     */
    public static XuanJi.adapter.qqbot.enums.QqApiErrorCode parseErrorCode(String responseBody) {
        if (responseBody == null || responseBody.isEmpty()) return null;
        try {
            ObjectNode errorJson = Json.parseObj(responseBody);
            int code = errorJson.path("code").asInt(0);
            return XuanJi.adapter.qqbot.enums.QqApiErrorCode.of(code);
        } catch (Exception e) {
            return null;
        }
    }
