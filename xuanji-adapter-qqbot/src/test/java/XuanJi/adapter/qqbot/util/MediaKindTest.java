...
class MediaKindTest {
    ...
    void payloadWithPngFilenameParsesAsImage() {
        ...
        XuanJiMessage chain = QqMessageConverter.fromQqPayload(raw);

        assertTrue(chain.has(XuanJiMessageElement.Image.class), "png 附件应解析为 Image 而非 File");
        assertFalse(chain.has(XuanJiMessageElement.File.class), "不应再落到 File 分支");
