...
chcp 65001 >nul
REM ============================================================
REM  XuanJi Build - Linux package (with Playwright render)
REM  Output:  xuanji-starter\target\xuanji-starter-1.0.0-SNAPSHOT.jar
REM  Usage:   double-click this script (JDK 25+ required)
REM  产物:    slim jar with only Linux Playwright driver
REM ============================================================

set "PROJ=%~dp0.."
cd /d "%PROJ%"

echo [XuanJi] Building Linux package (with Playwright)...
...
REM ---- slim driver to linux platform ----
set "JAR=xuanji-starter\target\xuanji-starter-1.0.0-SNAPSHOT.jar"
echo [XuanJi] Step 2/2: slimming Playwright driver for Linux...
python scripts\slim-jar.py "%JAR%" --platform linux
if errorlevel 1 (
