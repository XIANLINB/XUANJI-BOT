#!/usr/bin/env bash
# ============================================================
#  XuanJi Build - Linux 专用包（含 Playwright 渲染能力）
#  Output:  xuanji-starter/target/xuanji-starter-1.0.0-SNAPSHOT.jar
#  Usage:   bash scripts/build-playwright-linux.sh  (JDK 25+ required)
#  产物:    仅含 Linux 平台 Playwright driver 的精简 jar
# ============================================================
set -e
cd "$(dirname "$0")/.."

echo "[XuanJi] 构建 Linux 专用包（含 Playwright）..."
...
echo "[XuanJi] 步骤 2/2: 裁剪 Playwright driver 为 Linux 平台 ..."
"$PY" scripts/slim-jar.py "$JAR" --platform linux
