...
public class DefaultMediaRefResolver implements MediaRefResolver {
    ...
    @Override
    public MediaRef resolve(String rawRef, MediaType type) {
        String s = rawRef == null ? "" : rawRef.trim();
        MediaForm form = detect(s);
        return MediaRef.of(form, s, type);
    }

    static MediaForm detect(String s) {
        if (s.isEmpty()) {
            return MediaForm.PLATFORM_ID;
        }
        if (s.startsWith("http://") || s.startsWith("https://")) {
            return MediaForm.URL;
        }
        if (s.startsWith("data:")) {
            return MediaForm.DATA_URI;
        }
        if (s.startsWith("base64://")) {
            return MediaForm.BASE64;
        }
        if (s.startsWith("file://")) {
            return MediaForm.FILE_PATH;
        }
        // 本地绝对路径：/ 开头或 X:\ 盘符
