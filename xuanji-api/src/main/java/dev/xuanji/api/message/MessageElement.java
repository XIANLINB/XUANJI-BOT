...
public sealed interface MessageElement
    ...
    /** 图片 */
    record Image(String url, String fileMd5, int width, int height, long size) implements MessageElement {}
    ...
    /** 语音 */
    record Voice(String url, int duration) implements MessageElement {}

    /** 视频 */
    record Video(String url, int duration, int width, int height) implements MessageElement {
        public Video(String url) { this(url, 0, 0, 0); }
    }

    /** 文件 */
    record File(String url, String name, long size) implements MessageElement {}
