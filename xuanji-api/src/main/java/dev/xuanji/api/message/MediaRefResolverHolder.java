...
public final class MediaRefResolverHolder {
    ...
    public static MediaRef resolve(String platform, String rawRef, MediaType type) {
    ...
    /** 平台无关归一化（使用默认解析器）。 */
    public static MediaRef resolve(String rawRef, MediaType type) {
     ...
     * 下载到本地并返回 FILE_PATH 形态的 MediaRef；未启用 / 非 URL / 下载失败回退普通解析。
     * 开关判定：bot 级 &gt; 全局。
     */
    public static MediaRef resolveFile(String platform, String rawRef, MediaType type, String botKey) {
        MediaRef ref = resolve(platform, rawRef, type);
        if (ref.form() != MediaForm.URL) return ref; // 非 URL 无需下载
        var path = dev.xuanji.api.media.MediaFileDownloader.download(ref.raw(), type, botKey);
        if (path == null) return ref; // 未启用 / 超限 / 失败 → 保持 URL 形态
        return MediaRef.of(MediaForm.FILE_PATH, path.toString(), type);
    }

    /** 无 bot 上下文时按全局开关下载。 */
    public static MediaRef resolveFile(String platform, String rawRef, MediaType type) {
