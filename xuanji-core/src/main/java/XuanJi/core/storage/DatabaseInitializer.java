...
public class DatabaseInitializer {
    ...
    private void createAllTables() {
        ...
        // 1. XuanJiBot 实例注册表（instance_id = 真实 appId / selfId，跨平台唯一索引）
        jdbc.execute("""
        ...
        // 4.1 机器人归档（删 bot 时防误删；归档数据移入 data/_archive，记录 TTL 30 天后可恢复）
        jdbc.execute("""
            CREATE TABLE IF NOT EXISTS xuanji_bot_archive (
                id           BIGINT AUTO_INCREMENT PRIMARY KEY,
                platform     VARCHAR(16)  NOT NULL,
                instance_id  VARCHAR(128) NOT NULL,
                bot_key      VARCHAR(128),
                ...
                expire_at    TIMESTAMP,
                archive_dir  VARCHAR(512),
                data_size    BIGINT       DEFAULT 0,
                extra_json   TEXT
            )
        """);
        jdbc.execute("CREATE INDEX IF NOT EXISTS idx_bot_archive_expire ON xuanji_bot_archive (status, expire_at)");
        ...
        jdbc.execute("""
            CREATE TABLE IF NOT EXISTS xuanji_scheduler_job (
                ...
                repeat_mode     VARCHAR(16)  DEFAULT 'cron',
        ...
        """);
        // 兼容老库缺列（P1-R：一次性任务支持）
        try { jdbc.execute("ALTER TABLE xuanji_scheduler_job ADD COLUMN IF NOT EXISTS repeat_mode VARCHAR(16) DEFAULT 'cron'"); } catch (Exception ignored) {}
        // 兼容老库缺列（历史任务：软删除保留记录）
        try { jdbc.execute("ALTER TABLE xuanji_scheduler_job ADD COLUMN IF NOT EXISTS deleted BOOLEAN DEFAULT FALSE"); } catch (Exception ignored) {}
        try { jdbc.execute("ALTER TABLE xuanji_scheduler_job ADD COLUMN IF NOT EXISTS deleted_at BIGINT DEFAULT 0"); } catch (Exception ignored) {};
    ...
    /** 从 YAML 配置中注册 XuanJiBot 实例到 xuanji_bot 表 */
    private void registerBotsFromConfig() {
        var bots = robotProperties.getRobots();
        if (bots == null || bots.isEmpty()) {
            log.info("[DB] 未配置 XuanJiBot 实例，跳过注册");
            return;
        }
        ...
        for (var entry : bots.entrySet()) {
            ...
            try {
                ...
                log.info("[DB] XuanJiBot 已注册: platform={}, id={}, key={}", platform, appId, botKey);
            } catch (Exception e) {
                log.warn("[DB] XuanJiBot 注册失败: key={}, error={}", botKey, e.getMessage());
            }
        }
    }
}
