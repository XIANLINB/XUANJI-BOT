...
/**
 * XuanJiBot 数据源注册表 — 按需打开平台共享库 / per-bot 实例库，缓存 JdbcTemplate（HikariCP 连接池）。
 *
 * <pre>
...
public class BotDataSourceRegistry implements DisposableBean {
    ...
    public BotDataSourceRegistry(XuanJi.core.config.ConfigService configService) {
    ...
    /**
     * per-bot 日志库 JdbcTemplate（消息/事件流水，与业务数据分库自治）。
     *
     * <p>路径 {@code data/{platform}/{instanceId}/log/{instanceId}.mv.db}，
     * 独立 Hikari 池；删机器人时随 {@link #closeInstance} 一并关闭。
     */
    public JdbcTemplate forLog(String platform, String instanceId) {
        return compute("log:" + platform + ":" + instanceId,
                "./data/" + platform + "/" + instanceId + "/log/" + instanceId + ".log");
    }

    /** 释放某实例连接（含数据/日志两库，删机器人前置步骤），失败可忽略。 */
    public void closeInstance(String platform, String instanceId) {
        JdbcTemplate tpl = cache.remove("instance:" + platform + ":" + instanceId);
        if (tpl != null) {
            closeQuietly(tpl.getDataSource());
        }
        JdbcTemplate logTpl = cache.remove("log:" + platform + ":" + instanceId);
        if (logTpl != null) {
            closeQuietly(logTpl.getDataSource());
        }
    }
