...
/**
 * 验证 P1c：DedupStage 真正启用 xuanji_event_dedup 表做跨实例幂等去重。
...
public class DedupStageTest {
    ...
    private JdbcTemplate newH2() {
        ...
        j.execute("CREATE TABLE xuanji_event_dedup (event_id VARCHAR(128) PRIMARY KEY, platform VARCHAR(16) NOT NULL, create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP)");
        return j;
    }
    ...
    void sameEventIdIsDeduplicatedViaDb() {
        ...
        int n1 = jdbc.queryForObject("SELECT COUNT(*) FROM xuanji_event_dedup WHERE event_id='evt-1'", Integer.class);
        assertEquals(1, n1, "事件 ID 应写入 xuanji_event_dedup");
        ...
        int n2 = jdbc.queryForObject("SELECT COUNT(*) FROM xuanji_event_dedup WHERE event_id='evt-1'", Integer.class);
    ...
    void differentEventIdsBothPass() {
        ...
        int n = jdbc.queryForObject("SELECT COUNT(*) FROM xuanji_event_dedup", Integer.class);
        assertEquals(2, n);
    }
}
