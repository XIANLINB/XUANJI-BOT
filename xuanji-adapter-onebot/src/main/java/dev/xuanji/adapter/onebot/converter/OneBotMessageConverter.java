...
public final class OneBotMessageConverter {
    ...
    public static MessageChain toChain(JsonNode message) {
        ...
        for (JsonNode seg : message) {
            MessageElement el = OneBotMessageConverter.toElement(seg);
            if (el == null) continue;
            builder.add(el);
        }
    ...
    static MessageElement toElement(JsonNode seg) {
        ...
        return switch (type) {
            ...
            case "image" -> new MessageElement.Image(OneBotMessageConverter.firstNonBlank(d.path("url").asText(""), d.path("file").asText("")), d.path("file").asText(""), d.path("width").asInt(0), d.path("height").asInt(0), d.path("file_size").asLong(0L));
            case "face" -> new MessageElement.Face(d.path("id").asInt(0));
            case "record" -> new MessageElement.Voice(OneBotMessageConverter.firstNonBlank(d.path("url").asText(""), d.path("file").asText("")), d.path("duration").asInt(0));
            case "video" -> new MessageElement.Video(OneBotMessageConverter.firstNonBlank(d.path("url").asText(""), d.path("file").asText("")), 0, 0, 0);
            case "file" -> new MessageElement.File(OneBotMessageConverter.firstNonBlank(d.path("url").asText(""), d.path("file").asText("")), d.path("name").asText(d.path("file").asText("")), d.path("file_size").asLong(0L));
    ...
    private static MessageElement parseCqSegment(String body) {
        ...
        return OneBotMessageConverter.toElement((JsonNode)seg);
    }
