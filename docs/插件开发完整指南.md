...
> 完整可运行示例请直接看仓库自带的 **演示插件** `xuanji-plugin-demo`，以及 **接口测试插件** `xuanji-plugin-test`（群管动作的真实用例）。
...
<plugin>
    ...
    <configuration>
        <archive>
            <manifestEntries>
                <Plugin-Id>demo-plugin</Plugin-Id>          <!-- 插件唯一 ID -->
                <Plugin-Class>XuanJi.plugin.demo.DemoPlugin</Plugin-Class> <!-- 入口类 -->
            </manifestEntries>
        </archive>
    </configuration>
</plugin>
...
public class DemoPlugin extends XuanJiPluginBase {
    ...
    @XuanJiPlugin(id = "demo-plugin", name = "演示插件", version = "1.0.0",
        author = "XuanJi Team", description = "演示插件的全部能力",
...
```bash
# 打包（-am 必加：连带构建依赖模块）
mvn package -pl xuanji-plugin-demo -am -DskipTests
...
参考仓库自带的插件：
- **演示插件 `xuanji-plugin-demo`**：群聊/单聊/两者命令、参数 rest、角色权限、@模式、签到持久化、@GroupMessage/@PrivateMessage/@OnMessage、@GroupEvent 进群欢迎、@PrivateEvent、PluginServices、富媒体、Markdown，全部有可运行用例。
- **接口测试插件 `xuanji-plugin-test`**：群管动作真实用例——`#禁言`（@Arg 分钟 + 批量）、`#撤回`（@Arg 可选条数）、`#查看入群申请列表`/`#同意`/`#拒绝`/`#全部同意`/`#全部拒绝`、`#一键测试`（类型化返回）、`#按钮`~`#按钮4`（键盘测试）。
- **入群提示插件 `xuanji-plugin-groupnotify`**：监听入群/退群事件，配置提示语。
