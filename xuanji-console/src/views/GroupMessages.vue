...
import { ChatbubbleOutline, SearchOutline } from '@vicons/ionicons5'
  ...
  if (q) {
    rows = rows.filter(
      (r) =>
        ...
        String(r.GROUP_ID || '').toLowerCase().includes(q) // 群聊额外支持群号搜索
    )
  }
  return rows
})
...
const columns: DataTableColumns = [
  ...
  { title: '聊天类型', key: 'CHAT_TYPE', width: 100, render: () => h(NTag, { size: 'small', type: 'info', bordered: false }, () => '群聊') },
  {
  ...
  { title: '内容', key: 'CONTENT', minWidth: 300, ellipsis: { tooltip: true }, render: (row) => longCell(row.CONTENT) },
  { title: '群号', key: 'GROUP_ID', minWidth: 190, ellipsis: { tooltip: true }, render: (row) => longCell(row.GROUP_ID) },
  { title: '用户 ID', key: 'USER_ID', minWidth: 190, ellipsis: { tooltip: true }, render: (row) => longCell(row.USER_ID) },
  { title: '消息 ID', key: 'MSG_ID', minWidth: 240, ellipsis: { tooltip: true }, render: (row) => longCell(row.MSG_ID) }
]
